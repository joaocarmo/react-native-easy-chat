...
See [`App.tsx`](https://github.com/joaocarmo/react-native-easy-chat/blob/main/example/App.tsx) for a working demo!
...
- **`text`** _(String)_ - Input text; default is `undefined`, but if specified, it will override EasyChat's internal state (e.g. for redux; [see notes below](#notes-for-redux))
...
If you decide to specify a `text` prop, EasyChat will no longer manage its own internal `text` state and will defer entirely to your prop.
...
- For **Expo**, there are at least 2 solutions to fix it:
  ...
  - Append [`KeyboardAvoidingView`](https://facebook.github.io/react-native/docs/keyboardavoidingview) after EasyChat. This should only be done for Android, as `KeyboardAvoidingView` may conflict with the iOS keyboard avoidance already built into EasyChat, e.g.:
...
- If you plan to use `EasyChat` inside a `Modal`, see [#200](https://github.com/FaridSafi/react-native-gifted-chat/issues/200).
